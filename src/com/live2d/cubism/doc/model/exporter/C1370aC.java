package com.live2d.cubism.doc.model.exporter;

import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.Lambda;

import java.util.List;


/* renamed from: com.live2d.cubism.doc.model.exporter.aC */
/* loaded from: Live2D_Cubism.jar:com/live2d/cubism/doc/model/exporter/aC.class */
final class C1370aC extends Lambda implements Function1<EmMoc3Source, List<Integer>> {

    /* renamed from: a */
    public static final C1370aC f3544a = new C1370aC();


    C1370aC() {
        super();
    }

    /* renamed from: a */
    public List<Integer> invoke(EmMoc3Source receiver) {

        count = receiver.getEmModelSource().getEmCountInfo().getCounts().get(5);
        return receiver.getEmModelSource().getEmParameterSources().getDecimalPlaces();
    }
}
