package com.live2d.cubism.doc.model.exporter;


import java.util.ArrayList;
import java.util.List;

/* renamed from: com.live2d.cubism.doc.model.exporter.bG  reason: from Kotlin metadata */
/* loaded from: Live2D_Cubism.jar:com/live2d/cubism/doc/model/exporter/bG.class */
public final class EmArtMeshSources {


    /* renamed from: a */
    private final List<String> mID = new ArrayList<>();


    /* renamed from: b */
    private final List<Integer> mKeyformBindingBandSourcesIndex = new ArrayList<>();


    /* renamed from: c */
    private final List<Integer> mKeyformSourcesBeginIndex = new ArrayList<>();


    /* renamed from: d */
    private final List<Integer> mKeyformSourcesCount = new ArrayList<>();


    /* renamed from: e */
    private final List<Boolean> isVisible = new ArrayList<>();


    /* renamed from: f */
    private final List<Boolean> isEnable = new ArrayList<>();


    /* renamed from: g */
    private final List<Integer> mParentPartIndex = new ArrayList<>();


    /* renamed from: h */
    private final List<Integer> mParentDeformerIndex = new ArrayList<>();


    /* renamed from: i */
    private final List<Integer> mTextureNo = new ArrayList<>();


    /* renamed from: j */
    private final List<Byte> mDrawableFlag = new ArrayList<>();


    /* renamed from: k */
    private final List<Integer> mPositionIndexSourcesCount = new ArrayList<>();


    /* renamed from: l */
    private final List<Integer> mUVSourcesBeginIndex = new ArrayList<>();


    /* renamed from: m */
    private final List<Integer> mPositionIndexSourcesBeginIndex = new ArrayList<>();


    /* renamed from: n */
    private final List<Integer> mVertexCount = new ArrayList<>();


    /* renamed from: o */
    private final List<Integer> mDrawableMaskSourcesBeginIndex = new ArrayList<>();


    /* renamed from: p */
    private final List<Integer> mDrawableMaskSourcesCount = new ArrayList<>();


    /* renamed from: a */
    public List<String> getID() {
        return this.mID;
    }


    /* renamed from: b */
    public List<Integer> getKeyformBindingBandSourcesIndex() {
        return this.mKeyformBindingBandSourcesIndex;
    }


    /* renamed from: c */
    public List<Integer> getKeyformSourcesCount() {
        return this.mKeyformSourcesBeginIndex;
    }


    /* renamed from: d */
    public List<Integer> getKeyformSourcesBeginIndex() {
        return this.mKeyformSourcesCount;
    }


    /* renamed from: e */
    public List<Boolean> getVisible() {
        return this.isVisible;
    }


    /* renamed from: f */
    public List<Boolean> getEnable() {
        return this.isEnable;
    }


    /* renamed from: g */
    public List<Integer> getParentPartIndex() {
        return this.mParentPartIndex;
    }


    /* renamed from: h */
    public List<Integer> getParentDeformerIndex() {
        return this.mParentDeformerIndex;
    }


    /* renamed from: i */
    public List<Integer> getTextureNo() {
        return this.mTextureNo;
    }


    /* renamed from: j */
    public List<Byte> getDrawableFlag() {
        return this.mDrawableFlag;
    }


    /* renamed from: k */
    public List<Integer> getPositionIndexSourcesBeginIndex() {
        return this.mPositionIndexSourcesCount;
    }


    /* renamed from: l */
    public List<Integer> getUVSourcesBeginIndex() {
        return this.mUVSourcesBeginIndex;
    }


    /* renamed from: m */
    public List<Integer> getPositionIndexSourcesCount() {
        return this.mPositionIndexSourcesBeginIndex;
    }


    /* renamed from: n */
    public List<Integer> getVertexCount() {
        return this.mVertexCount;
    }


    /* renamed from: o */
    public List<Integer> getDrawableMaskSourcesCount() {
        return this.mDrawableMaskSourcesBeginIndex;
    }


    /* renamed from: p */
    public List<Integer> getDrawableMaskSourcesBeginIndex() {
        return this.mDrawableMaskSourcesCount;
    }

    /* renamed from: q */
    public int getSize() {
        return this.mID.size();
    }

}
